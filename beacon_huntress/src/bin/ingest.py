...
def build_raw(src_file, dest_parquet_file, overwrite = False, verbose = False):
    ...
    if overwrite == True:
        ...
        if os.path.exists(dest_parquet_file):
            logger.warning("Parquet file {} already exists, skipping file.".format(dest_parquet_file))
...
def build_filter_files(src_loc, dest_file = "", 
port_filter = None, port_exclude = False, 
src_filter = None, src_exclude = True, 
dest_filter  = None, dest_exclude = True, 
s_dns_filter = None, s_dns_exclude = True, 
d_dns_filter  = None, d_dns_exclude = True, 
match_filter = None, match_exclude = True, 
file_type = "parquet", overwrite = False, verbose = False):
    '''
    ...
    for root, dir, dir_f in os.walk(src_loc):
        ...
        for fname in dir_f:
            ...
            #CHECK IF FILE ALREADY EXISTS
            if overwrite == False and os.path.exists(dest_name) == True:
                logger.warning("File {} already exists, skipping".format(dest_name))
                continue
            elif overwrite == True:
...
def build_bronze_layer(src_loc, bronze_loc, dns_file = "", overwrite = False, verbose = False):
    ...
    Bronze data will tcp data only and will include source and destination DNS.\n
            ...
            if os.path.isfile(f):
                ...
                if overwrite == False:
                    if os.path.exists(par_name):
                        logger.warning("Parquet file {} already exists, skipping file.".format(par_name))
                        continue
                try:
                    is_new = True
