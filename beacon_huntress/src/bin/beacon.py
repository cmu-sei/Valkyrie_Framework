...
def _build_gold_beacons(delta_file, delta_column, gold_loc, beacon_conns, overwrite = False):
    ...
    for x in df_delta["source_file"].unique():
        ...
        # MODIFIED FOR WINDOWS OS FILES
        # PANDAS TREATS \ AS AN ESCAPE CHARACTER
        vals = df_delta[["src_row_id", "file_name", "datetime", "{}".format(delta_column)]].query("file_name == '{}'".format(fn_x))
        ...
        if os.path.exists(x) == True:
            ...
            df_og = pd.merge(org_df,vals[["src_row_id", "file_name", "datetime", "{}".format(delta_column)]],how="inner", left_on=['src_row_id', 'file_name'], right_on=['src_row_id', 'file_name'])
            logger.debug("Total records found {} for file {}".format(len(df_og),x))
        else:
            logger.info("Source File {} does not exist...skipping".format(x))
            df_og = df_delta[["sip", "dip", "port", "src_row_id", "file_name", "datetime", "{}".format(delta_column)]].query("file_name == '{}'".format(fn_x))
            df_og[["uid", "id.orig_p", "id.resp_p", "proto", "service", "duration", "orig_bytes", "resp_bytes", "conn_state", "local_orig", "local_resp", "missed_bytes", "history", "orig_pkts", "orig_ip_bytes", "resp_pkts", "resp_ip_bytes", "community_id"]] = np.NaN
    ...
    try:
        ...
        try:
            ...
            final_gold_df.sort_values(by=["id.orig_h", "id.resp_h", "ts"], inplace=True)
            #final_gold_df.sort_values(by=["sip", "dip", "datetime"], inplace=True)
            final_gold_df.to_parquet(gold_file, compression="snappy", use_deprecated_int96_timestamps=True)
            logger.info("Gold file {} created".format(gold_file))
        except BaseException as err:
...
def agglomerative_clustering(delta_file, delta_column, max_variance, min_records, cluster_factor, line_amounts, min_delta_time, gold_loc = "", cli_return = True, overwrite = False, verbose = False):
    '''
    ...
    # CLI_RETURN RESULTS
    if cli_return == True:
        if gold_file != None and gold_file != "":
            cli_results(
                gold_file = gold_file
                )
        else:
...
def dbscan_clustering(delta_file, delta_column, minimum_delta, spans = [], minimum_points_in_cluster = 4, minimum_likelihood = .70, gold_loc = "", cli_return = True, overwrite = False, verbose = False):
    '''
                ...
                for index, row in dbscan_cluster_data.iterrows():
                    if row['cluster'] > -1:
                        if row["likelihood"] > minimum_likelihood:
                            ...
                            Session # {} \n\
                            Src:    {} \n\
                            Dst:    {}".format(f["connection_id"].iloc[0], f["sip"].iloc[0], f["dip"].iloc[0]))
...
def dbscan_by_variance(delta_file, delta_column, avg_delta, conn_cnt = 5, span_avg = 15, variance_per = 10, minimum_likelihood = 70, gold_loc = "", cli_return = True, overwrite = False, verbose = False):
    '''
    ...
    for connection_id in connection_ids:
        ...
        if dbscan_cluster_data["likelihood"].any() > (minimum_likelihood / 100):
            ...
            Session # {} \n\
            Src:    {} \n\
            Dst:    {}".format(connection_id, max(f["sip"]), max(f["dip"])))      
...
def packet(delta_file, delta_column, avg_delta, conn_cnt = 5, min_unique_percent = 5, gold_loc = "", cli_return = True, overwrite = False, verbose = False):
    '''
    ...
    if cli_return == True:
        if gold_file != None and gold_file != "":
            # LEAVE FOR FUTURE VERSION
            # cli_results(
            #     gold_file = gold_file
            #     )

            # CURRENTLY THE GOLD FILE DOES NOT CONTAIN PACKET_UNQ_PER RUN THE NON-GOLD-CLI FOR CLI RESULTS
            _non_gold_cli(
                delta_file = delta_file,
                beacons_conns = beacons,
                sort_by = "packet_unq_per",
                pass_df = uni_df,
                asc = True
                )               
...
def cluster_conns(delta_file, delta_column, conn_cnt = 5, conn_group = 5, threshold = 60, gold_loc = "", cli_return = True, overwrite = False, verbose = False):
    '''
    ...
    if cli_return == True:
        if gold_file != None and gold_file != "":
            ...
            # CURRENTLY THE GOLD FILE DOES NOT CONTAIN GROUP_COUNT RUN THE NON-GOLD-CLI FOR CLI RESULTS
            _non_gold_cli(
                delta_file = delta_file,
                beacons_conns = beacons,
                sort_by = "group_count",
